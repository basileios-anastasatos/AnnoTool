...
function usage() {
	...
	OPTIONAL ARGUMENTS:                 DEFAULT
	 ...
	 -i <ignored_detections.idl[.gz]>
	 -o <output.atp>                    <detection.atp>
	 -s <number of score classes>       1
	 -D <doRPC.py additional arguments>

	 Either all or none of the -t, -f, and -m flags must be provided.

	 The -D flag must come at the very end.  All arguments following
	 the -D flag are passed to the doRPC.py script as they are.
	...
	COLOURS:
	 ...
	 [0;36mcyan   [0m ignored ground truth
	 [0;35mmagenta[0m ignored detection
...
USE_DORPC=yes;
DORPC_ARGS="";
while getopts ":hg:d:i:t:f:m:o:s:D" OPTION; do
    case "${OPTION:?}" in
        h) usage;;
        ...
        D) DORPC_ARGS=yes; shift $(expr $OPTIND - 1); break;;
